...
class CreateReelsController extends GetxController {
  ...
  Future<void> onSwitchCamera() async {
    ...
    if (isRecording == "stop") {
      Get.dialog(
          barrierDismissible: false, const LoadingWidget()); // Start Loading...
      if (isFlashOn) {
        onSwitchFlash();
      }
    ...
    try {
      if (cameraController != null && cameraController!.value.isInitialized) {
        Get.dialog(
            ...
            const LoadingWidget()); // Start Loading...
        onRestartAudio();
  ...
  Future<String?> onMergeAudioWithVideo(
    ...
    if (soundTime != 0 && videoTime != null && videoTime != 0) {
      ...
      // final command = '-i $videoPath -i $audioPath -t ... -c:v copy -c:a aac -strict experimental -map 0:v:0 -map 1:a:0 $path';
  ...
  Future<void> onSearchSound() async {
    ...
    if (searchController.text.trim().isNotEmpty) {
      ...
      searchSoundModel =
          await SearchSoundApi.callApi(searchString: searchController.text);
  ...
  Future<void> onGetAllSound() async {
    ...
    fetchSongModel = await FetchSongApi.callApi();
  ...
  Future<void> onGetFavoriteSound() async {
    ...
    fetchFavoriteSoundModel = await FetchFavoriteSongApi.callApi();
