...
import 'package:tingle/page/audio_wise_videos_page/model/preview_shorts_video_model.dart';
...
class AudioWiseVideosController extends GetxController {
  ...
  @override
  void onInit() {
    songId = Get.arguments[ApiParams.songId] ?? "";
  ...
  void onInitializeAudio() async {
    if (songLink != "") {
      try {
        await audioPlayer.setSource(UrlSource(songLink));
  ...
  Future<void> onClickReels(int index) async {
    ...
    Get.toNamed(AppRoutes.previewShortsVideoPage, arguments: {
      "index": index,
      "video": mainShorts,
      "previousPageIsAudioWiseVideoPage": true,
    });
  }
